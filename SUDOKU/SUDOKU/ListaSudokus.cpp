...
void tListaSudokus::mostrar_lista()
{
    ...
    for (int i = 0; i < numElems; i++)
    {
        ...
        for (int n = 1; n <= dim; n++)
        {
            ...
            for (int f = 0; f < dim; f++)
                for (int c = 0; c < dim; c++)
                    if (sudokus[i]->dame_celda(f, c).es_vacia() &&
                        sudokus[i]->posibles_valores(f, c) == n)
                        cont++;
            cout << "   celdas con " << n << " valores posibles: " << cont << "\n";
        }
    }
}
...
void tListaSudokus::guarda_lista_partidas(const string& nombreArchivo)
{
    ...
    for (int i = 0; i < numElems; i++)
    {
        int dim = sudokus[i]->dame_dimension();

        // cabecera: solo celdas originales
        arch << dim << "\n";
        for (int f = 0; f < dim; f++)
        {
            for (int c = 0; c < dim; c++)
            {
                tCelda cel = sudokus[i]->dame_celda(f, c);
                if (c > 0) arch << " ";
                arch << (cel.es_original() ? cel.dame_valor() : 0);
            }
            arch << "\n";
        }
